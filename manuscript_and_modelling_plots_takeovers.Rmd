...
participant.info <- read.csv(here::here("gaze_entropy_heterogenous/data/participant_info.csv")) %>%
  dplyr::select(1:5) %>%
  dplyr::rename("age" = "ï..Age", "ppid" = "Participant.ID")
...
predicted_braking_rt_ttc_plot <- ggplot(predicted_braking_rt_ttc, aes(x = .epred, fill = ttc_criticality_factor)) +
...
braking_rt_MER_TTC_plot <- ggplot(braking_rt_MER_TTC, aes(x = .value)) +
  stat_halfeye() +
  ylim(-.1, 1) +
  ylab("Density") +
...
fig4 <- posterior_braking_rt / (predicted_braking_rt_ttc_plot | braking_rt_MER_TTC_plot) + plot_layout(axes = "collect")
...
## Hands-off RT for presentations


```{r}
hands_off_rt <- response.vehicle.eye %>%
  dplyr::ungroup() %>%
  dplyr::filter(rt.outlier == FALSE, ho.outlier == FALSE, ho.rt != 0, collision == 0) %>%
  dplyr::mutate(ttc_criticality_factor = as.factor(ttc_criticality.x)) %>%
  ggplot(aes(x = rt, y = n_back)) +
  ...
  dplyr::mutate(ttc_criticality_factor = as.factor(ttc_criticality.x)), size = 2, height = .08, alpha = .3) +
  #geom_point(conditional_df_fitted[[3]], mapping = aes(x = estimate__, y = n_back), col = "white") +
  scale_y_discrete(labels = c('Monitoring','N-back + Monitoring'), guide = guide_axis(angle = 90)) +
  scale_color_brewer("Posterior predictive interval") +
  #xlim(.5, 3.5) +
  xlim(.5, 2) +
  xlab(expression(RT[B]~ "(s)")) +
  ...
  #facet_wrap(~ ttc_criticality_factor, labeller = labeller(ttc_criticality_factor = ttc_labs)) +
  theme_bw() +
  theme(legend.position = "none", legend.direction = "horizontal", axis.title.x = element_text(size = 11), axis.text.x = element_text(size = 11), axis.title.y = element_text(size = 11), axis.text.y = element_text(size = 8, face = "bold"), title = element_text(size = 18), legend.title = element_text(size = 12), legend.text = element_text(size = 12), legend.key = element_blank(), legend.key.width = unit(0.3, 'cm'), legend.key.size = unit(0.1, 'cm'), plot.title = element_text(size = 15, face = "bold"), strip.text = element_text(face = "bold", size = 10)) 

# plot saving
ggsave(here::here("takeover_performance_cogload_time_budget/Figures/hands_off_rt.png"), plot = hands_off_rt, width = 10, height = 7, units = 'cm', dpi = 300, type = 'cairo')
...
predicted_ho_rt_ttc_plot <- ggplot(predicted_ho_rt_ttc, aes(x = .epred, fill = ttc_criticality_factor)) +
  stat_halfeye(alpha = .5) +
  ylim(-.1, 1) +
  ylab("Density") +
  ...
  scale_fill_discrete(name = "TTC", labels = c("3 s", "5 s")) +
  theme_bw() +
  theme(legend.position = c(.85, .65), axis.title.x = element_text(size = 11), axis.text.x = element_text(size = 11), axis.title.y = element_text(size = 11), axis.text.y = element_text(size = 11), title = element_text(size = 18), legend.title = element_text(size = 12), legend.text = element_text(size = 12), legend.key = element_blank(), legend.key.width = unit(0.3, 'cm'), legend.key.size = unit(0.1, 'cm'), plot.title = element_text(size = 15, face = "bold"), strip.text = element_text(face = "bold", size = 10))


# MER of TTC on hands on RT
ho_rt_MER_TTC_plot <- ggplot(ho_rt_MER_TTC, aes(x = .value)) +
  stat_halfeye() +
  ylim(-.1, 1) +
  ylab("Density") +
  ...
  scale_fill_discrete(name = "TTC", labels = c("3 s", "5 s")) +
  theme_bw() +
  theme(legend.position = c(.90, .65), axis.title.x = element_text(size = 11), axis.text.x = element_text(size = 11), axis.title.y = element_text(size = 11), axis.text.y = element_text(size = 11), title = element_text(size = 18), legend.title = element_text(size = 12), legend.text = element_text(size = 12), legend.key = element_blank(), legend.key.width = unit(0.3, 'cm'), legend.key.size = unit(0.1, 'cm'), plot.title = element_text(size = 15, face = "bold"), strip.text = element_text(face = "bold", size = 10))

fig5 <- posterior_ho_rt / (predicted_ho_rt_ttc_plot | ho_rt_MER_TTC_plot)

# plot saving
ggsave(here::here("takeover_performance_cogload_time_budget/Figures/Fig5.tiff"), plot = fig5, width = 16, height = 12, units = 'cm', dpi = 300, type = 'cairo')
```
...
predicted_mean_decel_ttc_plot <- ggplot(predicted_mean_decel_ttc, aes(x = .epred, fill = ttc_criticality_factor)) +
  stat_halfeye(alpha = .5) +
  ylim(-.1, 1) +
  ylab("Density") +
  ...
  xlim(-4, -.5) +
  theme_bw() +
  theme(legend.position = c(.85, .60), axis.title.x = element_text(size = 11), axis.text.x = element_text(size = 11), axis.title.y = element_text(size = 11), axis.text.y = element_text(size = 11), title = element_text(size = 18), legend.title = element_text(size = 12), legend.text = element_text(size = 12), legend.key = element_blank(), legend.key.width = unit(0.3, 'cm'), legend.key.size = unit(0.1, 'cm'), plot.title = element_text(size = 15, face = "bold"), strip.text = element_text(face = "bold", size = 10))

# MER of TTC on mean deceleration
mean_decel_MER_TTC_plot <- ggplot(mean_decel_MER_TTC, aes(x = .value)) +
  stat_halfeye() +
    ylim(-.1, 1) +
  ylab("Density") +
...
# plot saving
ggsave(here::here("takeover_performance_cogload_time_budget/Figures/Fig6.tiff"), plot = fig6, width = 16, height = 12, units = 'cm', dpi = 300, type = 'cairo')
```
...
predicted_lat_acc_ttc_plot <- ggplot(predicted_lat_acc_ttc, aes(x = .epred, fill = ttc_criticality_factor)) +
  stat_halfeye(alpha = .5) +
  ylim(-.1, 1) +
  ylab("Density") +
...
predicted_SDLP_ttc_plot <- ggplot(predicted_SDLP_ttc, aes(x = .epred, fill = ttc_criticality_factor)) +
  stat_halfeye(alpha = .5) +
  ylim(-.1, 1) +
  ylab("Density") +
...
SDLP_MER_TTC_plot <- ggplot(SDLP_MER_TTC, aes(x = .value)) +
  stat_halfeye() +
  ylim(-.1, 1) +
  ylab("Density") +
...
# plot saving
ggsave(here::here("takeover_performance_cogload_time_budget/Figures/fig8.png"), plot = fig8, width = 16, height = 12, units = 'cm', dpi = 300, type = 'cairo')
```
...
sd_yaw_automation_lognormal_maximal <- brm(data = response.vehicle.eye %>%
                                             dplyr::filter(rt.outlier == FALSE, ho.outlier == FALSE, ho.rt != 0, collision == 0, ho.rt > .15) %>%
                             dplyr::mutate(ttc_criticality_factor = as.factor(ttc_criticality.x)) %>%
                             dplyr::mutate(ppid = as.factor(ppid)),
                           family = lognormal(),
                           ...
                           prior = c(prior(student_t(3, 0, 2.5), class = "Intercept"),
                                     prior(student_t(3, 0, 2.5), class = "b")),
                           iter = 5000, warmup = 2000, chains = 2, cores = 2, seed = 13, 
                                                   file = here::here("takeover_performance_cogload_time_budget/Models/sd_yaw_automation_lognormal_maximal"))
```
...
# model summary
print(summary(sd_yaw_automation_lognormal_maximal), digits = 5)
describe_posterior(sd_yaw_automation_lognormal_maximal, centrality = "mean")

# average predicted SD of yaw during automation with and without n-back
predicted_sd_yaw_automation_nback <- sd_yaw_automation_lognormal_maximal %>% 
...
sd_yaw_automation_MER_nback <- sd_yaw_automation_lognormal_maximal %>% 
  emmeans(~ n_back,
          epred = TRUE, re_formula = NA) %>% 
  contrast(method = "revpairwise") %>% 
  gather_emmeans_draws()
...
sd_yaw_manual_trunc_gaussian_maximal <- readRDS(here::here("takeover_performance_cogload_time_budget/Models/sd_yaw_manual_trunc_gaussian_maximal.rds"))

sd_yaw_manual_trunc_gaussian_maximal <- brm(data = response.vehicle.eye %>%
                             dplyr::filter(rt.outlier == FALSE, ho.outlier == FALSE, ho.rt != 0, collision == 0, ho.rt > .15) %>%
                             dplyr::mutate(ttc_criticality_factor = as.factor(ttc_criticality.x)) %>%
                             dplyr::mutate(ppid = as.factor(ppid)),
                           family = gaussian(),
                           bf(sd_yaw_manual | trunc(lb = 0) ~ n_back * ttc_criticality_factor + (n_back * ttc_criticality_factor| ppid)),
                           prior = c(prior(student_t(3, 0, 2.5), class = "Intercept"),
                                     prior(student_t(3, 0, 2.5), class = "b")),
                           iter = 5000, warmup = 2000, chains = 2, cores = 2, seed = 13, 
                           control = list(adapt_delta = 0.95),
                           file = here::here("takeover_performance_cogload_time_budget/Models/sd_yaw_manual_trunc_gaussian_maximal"))
```

## SD of horizontal gaze during manual driving: model predictions

```{r}
print(summary(sd_yaw_manual_trunc_gaussian_maximal), digits = 5)
describe_posterior(sd_yaw_manual_trunc_gaussian_maximal, centrality = "mean")

#### predicted posterior means and distribution ####
predicted_draws = empty_grid %>%
  add_predicted_draws(sd_yaw_manual_trunc_gaussian_maximal)

expected_values = empty_grid %>%
  add_epred_draws(sd_yaw_manual_trunc_gaussian_maximal)
...
predicted_sd_yaw_manual_ttc <- sd_yaw_manual_trunc_gaussian_maximal %>% 
  epred_draws(newdata = expand_grid(ttc_criticality_factor = c(as.factor(3), as.factor(5)),
                                    n_back = FALSE),
              re_formula = NA)
...
sd_yaw_manual_MER_TTC <- sd_yaw_manual_trunc_gaussian_maximal %>% 
  emmeans(~ ttc_criticality_factor,
          at = list(n_back = FALSE),
          epred = TRUE, re_formula = NA) %>% 
  contrast(method = "revpairwise") %>% 
...
predicted_sd_yaw_manual_nback <- sd_yaw_manual_trunc_gaussian_maximal %>% 
...
sd_yaw_manual_MER_nback <- sd_yaw_manual_trunc_gaussian_maximal %>% 
  emmeans(~ n_back,
...
# predicted means
predicted_sd_yaw_manual_ttc_plot <- ggplot(predicted_sd_yaw_manual_ttc, aes(x = .epred, fill = ttc_criticality_factor)) +
  stat_halfeye(alpha = .5) +
  ylim(-.1, 1) +
  ylab("Density") +
  xlab(expression("Predicted" ~ mu ~ "of" ~ SD[YAW]~ "(°)")) +
  ggtitle("B") +
  scale_fill_discrete(name = "TTC", labels = c("3 s", "5 s")) +
  xlim(8, 14) +
  theme_bw() +
...
sd_yaw_manual_MER_TTC_plot <- ggplot(sd_yaw_manual_MER_TTC, aes(x = .value)) +
  stat_halfeye() +
  ylim(-.1, 1) +
  ylab("Density") +
  xlab(expression(MER[SD[YAW]]~ "(°)")) +
  ggtitle("C") +
  scale_fill_discrete(name = "TTC", labels = c("3 s", "5 s")) +
  theme_bw() +
  theme(legend.position = c(.9, .7), axis.title.x = element_text(size = 11), axis.text.x = element_text(size = 11), axis.title.y = element_text(size = 11), axis.text.y = element_text(size = 11), title = element_text(size = 18), legend.title = element_text(size = 12), legend.text = element_text(size = 12), legend.key = element_blank(), legend.key.width = unit(0.3, 'cm'), legend.key.size = unit(0.1, 'cm'), plot.title = element_text(size = 15, face = "bold"), strip.text = element_text(face = "bold", size = 10))
...
# SD of horizontal gaze during maual driving - effect of N-back

```{r}
# predicted means
predicted_sd_yaw_manual_nback_plot <- ggplot(predicted_sd_yaw_manual_nback, aes(x = .epred, y = " ", fill = n_back)) +
  stat_halfeye(alpha = .5) +
  ylab(NULL) +
  xlab(expression("Predicted" ~ mu ~ "of" ~ SD[YAW]~ "(°)")) +
  scale_fill_discrete(name = "Task", labels = c("Monitoring", "N-back + monitoring")) +
  ggtitle(expression(SD[YAW]~": post-transition manual drive")) +
  xlim(8, 17) +
  theme_bw() +
  theme(legend.position = c(.70, .70), axis.title.x = element_text(size = 11), axis.text.x = element_text(size = 11), axis.title.y = element_text(size = 11), axis.text.y = element_text(size = 11), title = element_text(size = 10), legend.title = element_text(size = 12), legend.text = element_text(size = 12), legend.key = element_blank(), legend.key.width = unit(0.3, 'cm'), legend.key.size = unit(0.1, 'cm'), plot.title = element_text(size = 10, face = "bold"), strip.text = element_text(face = "bold", size = 10))

# Average marginal effect for SD of yaw
sd_yaw_manual_MER_nback_plot <- ggplot(sd_yaw_manual_MER_nback, aes(x = .value, y = " ")) +
  stat_halfeye() +
  ylab(NULL) +
  xlab(expression(MER[SD[YAW]]~ "(°)")) +
  #scale_fill_discrete(name = "TTC", labels = c("3 s", "5 s")) +
...
fig99 <- (predicted_sd_yaw_manual_nback_plot | sd_yaw_manual_MER_nback_plot)

# plot saving
ggsave(here::here("takeover_performance_cogload_time_budget/Figures/fig99.tiff"), plot = fig99, width = 18, height = 6, units = 'cm', dpi = 300, type = 'cairo')
...
summary(sd_yaw_manual_trunc_gaussian_maximal)[["random"]][["ppid"]][2,1] / summary(sd_yaw_manual_trunc_gaussian_maximal)[["fixed"]][2,1]
...
ppid.effects <- as.data.frame(ranef(sd_yaw_manual_trunc_gaussian_maximal)) %>%
  dplyr::select(ppid.Estimate.n_backTRUE) %>%
  dplyr::mutate(ranef = ppid.Estimate.n_backTRUE + summary(sd_yaw_manual_trunc_gaussian_maximal)[["fixed"]][2,1]) %>%
  tibble::rownames_to_column("ppid")

ppid.effects$x <- 0

# CI interval for average effect
bayes.credible.intervals <- as.data.frame(summary(sd_yaw_manual_trunc_gaussian_maximal)[["fixed"]][3:4])

# select random effects parameter 
ran.effects <- summary(sd_yaw_manual_trunc_gaussian_maximal)[["random"]][["ppid"]][2,1]
...
strip_plot <- ggplot() +
  ...
  ylab(expression("N-back effect on" ~ mu ~ "of" ~ SD[YAW] ~ "during manual driving (°)")) +
  xlab("") + 
  ggtitle("A") +
  xlim(-1, 1) +
  ...
  geom_hline(aes(yintercept = summary(sd_yaw_manual_trunc_gaussian_maximal)[["fixed"]][2,1], linetype = "beta_1"), size = 1.5, color = "black") +
  geom_hline(aes(yintercept = summary(sd_yaw_manual_trunc_gaussian_maximal)[["fixed"]][2,1] + 1.96 * ran.effects, linetype = "95% HI"), size = 1.5, color = viridis(5)[2]) +
  geom_hline(aes(yintercept = summary(sd_yaw_manual_trunc_gaussian_maximal)[["fixed"]][2,1] - 1.96 * ran.effects, linetype = "95% HI"), size = 1.5, color = viridis(5)[2]) +
...
summary(sd_yaw_manual_trunc_gaussian_maximal)[["fixed"]][2,1] + 1.96 * ran.effects
summary(sd_yaw_manual_trunc_gaussian_maximal)[["fixed"]][2,1] - 1.96 * ran.effects


# participant 3, beta_n = -0.022
p_3 <- ggplot(dat.eye.manual %>%
  ...
  scale_colour_viridis_c(option = "C") +
  ggtitle(label = "B",
          subtitle = "Participant 3," ~ beta[N[3]] ~ "= -.022") +
  xlab("Yaw (°)") +
  ylab("Pitch (°)") +
  xlim(-50, 50) +
  scale_y_continuous(limits = c(-32, 20), breaks = seq(-30, 20, 15)) +
...
p_14 <- ggplot(dat.eye.manual %>%
  ...
  scale_colour_viridis_c(option = "C") +
  ggtitle(label = " ",
          subtitle = "Participant 14," ~ beta[N[14]] ~ "= .052") +
  xlab("Yaw (°)") +
  ylab("Pitch (°)") +
  xlim(-50, 50) +
  scale_y_continuous(limits = c(-32, 20), breaks = seq(-30, 20, 15)) +
...
p_7 <- ggplot(dat.eye.manual %>%
  ...
  scale_colour_viridis_c(option = "C") +
  ggtitle(label = "C",
          subtitle = "Participant 7," ~ beta[N[7]] ~ "= -2.424") +
  xlab("Yaw (°)") +
  ylab("Pitch (°)") +
  xlim(-50, 50) +
  scale_y_continuous(limits = c(-32, 20), breaks = seq(-30, 20, 15)) +
...
p_13 <- ggplot(dat.eye.manual %>%
  ...
  scale_colour_viridis_c(option = "C") +
  ggtitle(label = " ",
          subtitle = "Participant 13," ~ beta[N[13]] ~ "= -2.317") +
  xlab("Yaw (°)") +
  ylab("Pitch (°)") +
  xlim(-50, 50) +
  scale_y_continuous(limits = c(-32, 20), breaks = seq(-30, 20, 15)) +
...
## Example participant how has carryover effects - for presentations

```{r}
l.labs <- c("Monitoring", "N-back + Monitoring")
names(l.labs) <- c("FALSE", "TRUE")

p49 <- ggplot(dat.eye.manual %>%
         dplyr::filter(ppid == 7), mapping = aes(x = yaw, y = pitch)) +
  geom_density_2d(aes(col = ..level..), alpha = .5, binwidth = .00003) +
  facet_wrap(~ n_back, labeller = labeller(n_back = l.labs)) +
  scale_colour_viridis_c(option = "C") +
  ggtitle("Gaze distribution during post-transition manual driving following:") +
  xlab("Yaw (°)") +
  ylab("Pitch (°)") +
  xlim(-50, 50) +
  scale_y_continuous(limits = c(-32, 20), breaks = seq(-30, 20, 15)) +
...
ggsave(here::here("takeover_performance_cogload_time_budget/Figures/fig12.png"), plot = p49, width = 12, height = 7.5, units = 'cm', dpi = 300, type = 'cairo')
```

# Does age impact effect of N-back

```{r}
sd_yaw_manual_trunc_gaussian_maximal_age <- brm(data = response.vehicle.eye %>%
                             dplyr::filter(rt.outlier == FALSE, ho.outlier == FALSE, ho.rt != 0, collision == 0, ho.rt > .15) %>%
                             dplyr::mutate(ttc_criticality_factor = as.factor(ttc_criticality.x)) %>%
                             dplyr::mutate(ppid = as.factor(ppid)),
                           family = gaussian(),
                           bf(sd_yaw_manual | trunc(lb = 0) ~ n_back * ttc_criticality_factor * age.c + (n_back | ppid)),
                           prior = c(prior(student_t(3, 0, 2.5), class = "Intercept"),
                                     prior(student_t(3, 0, 2.5), class = "b")),
                           iter = 5000, warmup = 2000, chains = 2, cores = 2, seed = 13, 
                           control = list(adapt_delta = 0.95),
                           file = here::here("takeover_performance_cogload_time_budget/Models/sd_yaw_manual_trunc_gaussian_maximal_age"))

sd_yaw_manual_trunc_gaussian_maximal_age <- readRDS(here::here("takeover_performance_cogload_time_budget/Models/sd_yaw_manual_trunc_gaussian_maximal_age.rds"))

print(summary(sd_yaw_manual_trunc_gaussian_maximal_age), digits = 5)
describe_posterior(sd_yaw_manual_trunc_gaussian_maximal_age)


#Get interaction coefficient
nXa.coeff <- fixef(sd_yaw_manual_trunc_gaussian_maximal_age)[6]

#Get residual variance of valence random effect in model with n_back
residvalvar <- VarCorr(sd_yaw_manual_trunc_gaussian_maximal_age)$ppid$sd[2, 1]^2

#Calculate the implied total valence random effect variance
imptotalvalvar <- nXa.coeff^2*sd(response.vehicle.eye$age.c)^2 + residvalvar

## Variance in mu difference explained by age
1 - (residvalvar/imptotalvalvar)

# computing random effects
ageranef <- as.data.frame(ranef(sd_yaw_manual_trunc_gaussian_maximal_age)) %>%
...
#Create dataset with one line per person with age score 
ageranef <- merge(ageranef, response.vehicle.eye, by = 'ppid')
ageranef <- ageranef %>%
  dplyr::group_by(ppid) %>%
  dplyr::slice(1) %>%
...
ranef.age.pred.tot <- fixef(sd_yaw_manual_trunc_gaussian_maximal_age)[2] + # fixed effect for n-back
  fixef(sd_yaw_manual_trunc_gaussian_maximal_age)[6] * # fixed effect for n-back X age interaction
  ageranef$age.c + # age 
  as.data.frame(ranef(sd_yaw_manual_trunc_gaussian_maximal_age))$ppid.Estimate.n_backTRUE # random effect slopes

fig13 <- ggplot(ageranef, aes(age.c, ranef.age.pred.tot)) + 
  ...
  ylab(expression("Implied total heterogeneity of "~ SD[YAW])) +
  #ylim(-.15, .1) +
  theme_bw() +
...
ggsave(here::here("takeover_performance_cogload_time_budget/Figures/fig13.tiff"), plot = fig13, width = 8, height = 8, units = 'cm', dpi = 300, type = 'cairo')
...
fig13 <- ggplot(ageranef, aes(age.c, ranef.age.pred.tot)) + 
  ...
  geom_jitter(height = 0, width = 0, size = 4,
              shape = 21, colour = "black", fill = viridis(5)[3], alpha = .95, stroke = 1) + 
  xlab("Age (mean centered)") +
  ylab(expression("Average effect N-back of "~ SD[YAW])) +
  #ylim(-.15, .1) +
  theme_bw() +
  theme(legend.position = "bottom", legend.direction = "horizontal", axis.title.x = element_text(size = 11), axis.text.x = element_text(size = 15), axis.title.y = element_text(size = 11), axis.text.y = element_text(size = 15), title = element_text(size = 18), legend.title = element_text(size = 12), legend.text = element_text(size = 12), legend.key = element_blank(), legend.key.width = unit(0.3, 'cm'), legend.key.size = unit(0.1, 'cm'), plot.title = element_text(size = 7.5, face = "bold"), strip.text = element_text(face = "bold", size = 10))


ggsave(here::here("takeover_performance_cogload_time_budget/Figures/fig13.png"), plot = fig13, width = 10, height = 7, units = 'cm', dpi = 300, type = 'cairo')
```
