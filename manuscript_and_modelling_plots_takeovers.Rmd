...
```{r}
# average effect of TTC on braking RT 
...
# plot saving
ggsave(here::here("takeover_performance_cogload_time_budget/Figures/Fig4.tiff"), plot = fig4, width = 16, height = 11, units = 'cm', dpi = 300, type = 'cairo')
```
...
fig7 <- posterior_mean_lat_acc / avg_lat_acc_plot

# plot saving
ggsave(here::here("takeover_performance_cogload_time_budget/Figures/Fig7.tiff"), plot = fig7, width = 16, height = 11, units = 'cm', dpi = 300, type = 'cairo')
```
...
# initial model of SDLP
sdlp_skew_normal_maximal <- brm(data = response.vehicle.eye %>%
                                  dplyr::filter(rt.outlier == FALSE, ho.outlier == FALSE, ho.rt != 0, collision == 0, ho.rt > .15) %>%
                                  dplyr::mutate(ttc_criticality_factor = as.factor(ttc_criticality.x)) %>%
                                  dplyr::mutate(ppid = as.factor(ppid)),
                              family = skew_normal(),
                              ...
                              bf(sdlp ~ n_back * ttc_criticality_factor + (n_back * ttc_criticality_factor| ppid)),
                                   control = list(adapt_delta = 0.95),
                                   iter = 5000, warmup = 2000, chains = 2, cores = 2, seed = 13, 
                                   file = here::here("takeover_performance_cogload_time_budget/Models/sdlp_skew_normal_maximal"))

print(summary(sdlp_skew_normal_maximal), digits = 5)
describe_posterior(sdlp_skew_normal_maximal, centrality = "mean")

pp_check(sdlp_skew_normal_maximal, type = "stat_grouped", stat = "mean", group = "ttc_criticality_factor")
pp_check(sdlp_skew_normal_maximal, type = "stat_grouped", stat = "mean", group = "n_back")

pp_check(sdlp_skew_normal_maximal, type = "dens_overlay_grouped", group = "n_back", ndraws = 100)
pp_check(sdlp_skew_normal_maximal, type = "dens_overlay_grouped", group = "ttc_criticality_factor", ndraws = 100)
...
describe_posterior(sd_yaw_manual_gaussian_maximal)
```
...
describe_posterior(sd_yaw_automation_gaussian_maximal)
