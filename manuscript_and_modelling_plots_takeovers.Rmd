...
Tidy up longitudinal jerk modelling 

Model lateral jerk to see if model fit and inferences are still good (with maximal random effects)

Add random interactions to all the other models 

Adjust the plots and tables

Send to co-authors and start on supplemental material.

```{r}
# distribution of negative longitudinal jerk
ggplot(response.vehicle.eye %>%
         dplyr::filter(rt.outlier == FALSE, ho.outlier == FALSE, ho.rt != 0, ho.rt > .15, collision == 0) %>%
         dplyr::mutate(ttc_criticality_factor = as.factor(ttc_criticality.x)) %>%
         dplyr::mutate(ppid = as.factor(ppid)), mapping = aes(max_long_jerk)) +
  geom_histogram() +
...
max_long_mixture_weak_priors  <- readRDS(here::here("ITS/Work package 6.5 (HiDrive)/TME study/Analysis/max_long_mixture_weak_prior.rds"))
...
pp_check(max_long_mixture_6, type = "stat_grouped", stat = "mean", group = "n_back")

pp_check(max_long_mixture_6, type = "dens_overlay_grouped", group = "n_back", ndraws = 100)
pp_check(max_long_mixture_6, type = "dens_overlay_grouped", group = "ttc_criticality_factor", ndraws = 100)

grand_mean_n_back_dist <- max_long_mixture_2_ttc %>% 
  epred_draws(newdata = expand_grid(ttc_criticality_factor = c(as.factor(3), as.factor(5))),
              re_formula = NA)
...
# model of maximum longitudinal acceleration with hella weak priors. To check if the model is blowing up cause if lack of priors constraining parameter values
max_long_jerk_skewnormal_reg_priors_maximal <- brm(data = response.vehicle.eye %>%
                                             dplyr::filter(rt.outlier == FALSE, ho.outlier == FALSE, ho.rt != 0, ho.rt > .15, collision == 0) %>%
                                             dplyr::mutate(ttc_criticality_factor = as.factor(ttc_criticality.x)) %>%
                                             dplyr::mutate(ppid = as.factor(ppid)),
                                           family = skew_normal(),
                                           ...
                                           control = list(adapt_delta = 0.99999999999),
                                           iter = 5000, warmup = 2000, chains = 2, cores = 2, seed = 13,
                                           file = "max_long_jerk_skewnormal_reg_priors_maximal")


print(summary(max_long_jerk_skewnormal_reg_priors_maximal), digits = 5)
...
pp_check(max_long_jerk_skewnormal_reg_priors_maximal, type = "dens_overlay_grouped", group = "n_back.x", ndraws = 100)
pp_check(max_long_jerk_skewnormal_reg_priors_maximal, type = "dens_overlay_grouped", group = "ttc_criticality_factor", ndraws = 100)
...
# prior for beta_C i.e., the change in max long when TTC = 5 s
prior(uniform(10, 11)) %>% 
  parse_dist() %>% 
  ggplot(aes(xdist = .dist_obj, y = prior)) + 
  stat_halfeye(.width = c(.5, .99)) +
...
# regularising priors for the rest of the parameters
prior(normal(9, 1)) %>% 
  parse_dist() %>% 
  ggplot(aes(xdist = .dist_obj, y = prior)) + 
  stat_halfeye(.width = c(.5, .99)) +
...
global_mean_ttc_sdyaw <- ggplot(grand_mean_ttc_dist ,aes(x = .epred, y = " ", fill = ttc_criticality_factor)) +
  stat_halfeye(alpha = .5) +
  ylab(NULL) +
  xlab(expression("Predicted mean"~ SD[YAW]~ "(°)")) +
  ggtitle("B") +
  scale_fill_discrete(name = "TTC", labels = c("3 s", "5 s")) +
  xlim(8, 14) +
  theme_bw() +
